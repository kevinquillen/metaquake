...
def_t *def_for_type[8] =
    { &def_void, &def_string, &def_float, &def_vector, &def_entity, &def_field,
...
NEW: Merges multiple quoted strings into one. "abc" "123" -> "abc123"
==============
*/
void PR_LexString(void)
{
	...
	for(;;)
	{
		...
		else if (c == '\"')
		{
			...
			return;
			#if 0
			//Lex the whitespace, then check if we found another open quote
			//if so, then 
			PR_LexWhitespace();
			if(*pr_file_p == '\"')
			{
				//skip open quote, restart loop
				pr_file_p++;
				//foundmany = 1;
				len = 0;
				continue;
